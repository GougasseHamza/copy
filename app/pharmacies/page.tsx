...
export default function PharmaciesPage() {
  ...
  return (
    <div className="container-custom py-8">
      <div className="mb-8">
        <h1 className="text-4xl font-bold mb-2">Pharmacies à proximité</h1>
        <p className="text-muted-foreground">Trouvez les pharmacies ouvertes près de chez vous</p>
      </div>
      ...
      <div className="grid md:grid-cols-3 gap-6">
        {/* Pharmacy List/Map */}
        <div className={viewMode === 'map' ? 'md:col-span-2' : 'md:col-span-3'}>
          {/* List View */}
          {viewMode === 'list' && (
            <div className="space-y-4">
              {filteredPharmacies.map((pharmacy) => (
                <div
                  key={pharmacy.id}
                  className="group bg-card rounded-lg border p-6 hover:shadow-lg transition-all duration-300 cursor-pointer"
                ...
                >
                  <div className="flex gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2">
                        <Link href={`/pharmacy/${pharmacy.id}`} className="flex-1 text-center px-4 py-2 bg-primary text-primary-foreground rounded-md hover:bg-primary/90 transition-colors">
                          Voir les produits
                        </Link>
            ...
            <div className="bg-card rounded-lg border overflow-hidden" style={{ height: '600px' }}>
              <div className="w-full h-full flex items-center justify-center bg-muted">
                <div className="text-center">
                  ...
                  <p className="text-sm text-muted-foreground mt-2">Intégration en cours de développement</p>
                </div>
              </div>
            </div>
          )}
        ...
        {viewMode === 'map' && selectedPharmacy && (
          <div className="bg-card rounded-lg border p-6">
            <h3 className="text-xl font-semibold mb-4">{selectedPharmacy.name}</h3>
            <div className="space-y-4">
              <div>
              ...
              <div>
                <p className="text-sm text-muted-foreground mb-1">Évaluation</p>
                <div className="flex items-center gap-2">
                  <div className="flex">
                    {[1, 2, 3, 4, 5].map((i) => (
                      <svg
                  ...
                  <span className="text-sm">{selectedPharmacy.rating} ({selectedPharmacy.reviewCount} avis)</span>
                </div>
              </div>
              <Link href={`/pharmacy/${selectedPharmacy.id}`} className="block w-full text-center px-4 py-2 bg-primary text-primary-foreground rounded-md hover:bg-primary/90 transition-colors">
                Voir les détails
              </Link>
            </div>
          </div>
